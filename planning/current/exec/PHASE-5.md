...
threshold OR p75/p90), and fire native desktop notifications for new failures / slow CI / new
comments.
**Exit criteria:** slow runs are flagged with a human-readable reason; a real desktop
notification fires on the dev OS; baseline math verified; Actions timing verified live.
...
| **PAT** (B1) | Actions API | keychain | 200 | — |
| **Sandbox repo with workflow runs of varying duration** (B2) | baseline + slow-flag proof | you ensure a workflow has run several times (some slow) | `GET /actions/runs` non-empty | — |
...
**Blocker note:** B4 — headless CI has no real desktop. Pipeline starts dbus + a daemon and
asserts the D-Bus `Notify` call fires (dbus-mock). A true on-screen render is verified on the
dev desktop (DISPLAY present); if neither is available → flag partial.

## 3. Execution map (Step 6.4)

```mermaid
flowchart TD
    ENVS["env-setup (devops-agent)\nPAT + runs-bearing sandbox + dbus/notif daemon\nGATE"] --> FORK{{orchestrator spawns lanes}}
    ...
    AGG["AGGREGATION (orchestrator)\nLIVE: real run timings; baseline computed; slow run flagged; notification fires (dbus-mock + dev-desktop)"] --> ENF["ENFORCEMENT GATE"]
...
|---|---|---|---|---|---|---|
| env-setup | devops-agent | §2 incl. notif daemon | PAT, repo, host | ready env + daemon | MASTER §4 | gate |
...
| app-notify | code-agent | `notify-rust` dispatcher + de-dupe (don't re-notify same event) | poller-timing | notifications | no token/PII in body; user-toggle | poller-timing |
...
| atdd-timing | test-agent (ATDD) | acceptance: slow run → flagged+notified; failed run → notified | §7 | live acceptance + dbus assertion | real sandbox + daemon | app-notify |
...
- **ATDD:** a known-slow sandbox run is flagged with a sensible reason and triggers a
...
Boundaries: **Actions API** (live timing) and **desktop notification** (real D-Bus `Notify`
fired, asserted; on-screen render confirmed on dev desktop). Unverifiable notification path
(no daemon, no desktop) = flagged partial, never stubbed.

## 8. Gap report (Step 6.9)
- **B2**: needs multiple historical runs to form a baseline — if too few, baseline mode degrades
  to threshold; flagged. **B4**: headless notification verified via dbus-mock; full visual
  confirmation requires the dev desktop.

## 9. Debug & retry (Step 6.10)
Per [MASTER §8](./MASTER.md). Likely: per-OS notification backend differences (notify-rust mac
backend) → subagent handles per-OS; flaky daemon → re-run env-setup.

## 10. Aggregation & gate
orchestrator: live timing + baseline + flag + notification proof → enforcement + light security
→ session update → Phase 5 closed.
