...
**Exit criteria:** filters compose correctly over real data; poll cadence visibly changes on
window focus/blur; settings persist; empty/error states render; idle CPU measured negligible.
...
| **PAT + populated sandbox** (B1/B2) | real items to filter | keychain/repo | non-empty feed | — |
...
**Blocker note:** B5 — true focus/blur requires a window manager; headless xvfb can simulate via
events but real verification is on the dev desktop. Idle-CPU measured with pidstat over an idle
window.
...
flowchart TD
    ...
    AGG["AGGREGATION (orchestrator)\nLIVE: compose filters on real feed; observe cadence change on blur; measure idle CPU (pidstat)"] --> PERF["performance-agent: idle-CPU + redraw-on-event audit"]
...
|---|---|---|---|---|---|---|
| env-setup | devops-agent | §2 incl. WM + pidstat | PAT, repo, host | ready env | MASTER §4 | gate |
...
| atdd-polish | test-agent (ATDD) | acceptance: compose filters on real feed; blur slows poll; idle CPU below budget | §7 | live acceptance + pidstat reading | real feed + desktop | app-filterbar, app-focus |
...
- **ATDD:** toggling chips narrows the feed correctly across combined dimensions; blurring the
  window measurably lengthens the poll interval; restoring focus shortens it; idle CPU under the
  NFR2 budget (pidstat).
...
Boundaries: **window-manager focus/blur ↔ poller cadence** (verified by observing the actual
poll interval change on real focus events) and filters over the **live feed**. Idle-CPU is the
NFR2 acceptance evidence, measured with pidstat — not asserted by claim.

## 8. Gap report (Step 6.9)
- **B5**: true blur needs a WM; headless run simulates events but full proof is on dev desktop.
  Idle-CPU budget value to confirm with you (PRD OQ "default cadence values").
